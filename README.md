...
7. A dialog appears. Under *Site name*, enter **PassCore Website**. Under *Application pool* click on *Select* and ensure you select **PassCore Application Pool**. Under *Physical path*, click on the ellispsis *(...)*, navigate to the folder where you extracted PassCore and select the **wwwroot** folder. **Important:** Make shure the Physical path points to the <code>wwwroot</code> folder and not to its parent folder which is the one containing <code>approot</code>, <code>logs</code> and <code>wwwroot</code> folders.
